...
export default function CartaPage() {
  ...
  return (
    <main className="min-h-screen pb-40 bg-ink-950 text-ink-50">
      ...
      {/* ── Header ── */}
      <header className="sticky top-0 z-40 bg-ink-950/80 backdrop-blur-md px-5 pt-6 pb-4 flex items-center justify-between">
        ...
        {/* Sorting Nav */}
        <div className="flex items-center gap-1 bg-white/5 p-1 rounded-lg border border-white/10 text-ink-400">
          <button
            onClick={() => setSortBy("price_asc")}
            className={`flex items-center justify-center w-8 h-8 rounded-md transition-all ${
              sortBy === "price_asc" ? "bg-white/20 text-ink-50" : "hover:text-ink-200"
            }`}
            title="Menor a mayor precio"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="m3 16 4 4 4-4"/><path d="M7 20V4"/><path d="M11 4h4"/><path d="M11 8h7"/><path d="M11 12h10"/></svg>
          </button>
          <button
            onClick={() => setSortBy("price_desc")}
            className={`flex items-center justify-center w-8 h-8 rounded-md transition-all ${
              sortBy === "price_desc" ? "bg-white/20 text-ink-50" : "hover:text-ink-200"
            }`}
            title="Mayor a menor precio"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="m3 8 4-4 4 4"/><path d="M7 4v16"/><path d="M11 4h10"/><path d="M11 8h7"/><path d="M11 12h4"/></svg>
          </button>
        </div>
      </header>
