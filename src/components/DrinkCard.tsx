...
import { Plus, Minus, LucideIcon, Candy, GlassWater, Citrus, Wind, Star, Zap, Leaf } from 'lucide-react';

export type Flavor = string;

const FLAVOR_ICONS: Record<Flavor, LucideIcon> = {
  'Dulce': Candy,
  'Amargo': GlassWater,
  'Cítrico': Citrus,
  'Refrescante': Wind,
...
export default function DrinkCard({ 
  ...
  return (
    <div onClick={onAdd} className={`group relative flex flex-col p-4 rounded-2xl bg-[#0f172a] border cursor-pointer active:scale-[0.98] transition-all duration-200 ${
      isTrending 
        ...
        {flavors.length > 0 && (
          <div className="flex flex-wrap gap-3 mb-4">
            {flavors.map(f => {
              ...
              return (
                <div key={f} className="flex items-center gap-1 text-slate-300">
                  {FlavorIcon && <FlavorIcon size={12} className="text-[#38bdf8]" />}
                  <span className="text-[10px] font-light tracking-wide">{f}</span>
                </div>
              );
            })}
          </div>
         ...
         <div className="flex items-center gap-2">
           {quantity > 0 ? (
             ...
             >
               ...
               <span className="text-white font-mono w-6 text-center text-xs font-bold">{quantity}</span>
               <button 
                onClick={(e) => {
                  e.stopPropagation();
                  if (onAdd) onAdd();
