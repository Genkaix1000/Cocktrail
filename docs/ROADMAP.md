...
- [x] **Tabla `audit_logs` aplicada en Supabase Cloud (R13, 2026-07-13)**: existía localmente
  (`supabase/migrations/20260629201500_create_audit_logs.sql`) pero nunca se había aplicado en el
  proyecto cloud real. Se corrió el SQL a mano en el SQL Editor de Supabase Cloud. Verificado con
  `GET /rest/v1/audit_logs?select=id&limit=1` contra la cloud real: `200 []` (antes devolvía
...
- [x] **Hardening de Kong / demo keys hardcodeadas** (R7) → spec
...
- [x] **R12 resuelto (2026-07-13)**: eliminados de `useAdminAnalytics`/`lib/analytics.ts` los 15
  ...
  verde.
- [x] **R13 resuelto (2026-07-13)**: migración `audit_logs` aplicada a mano en el SQL Editor de
  Supabase Cloud, verificada con `GET /rest/v1/audit_logs` contra la cloud real.
...
| R7 | `supabase/docker/kong.yml` usaba las **demo keys públicas** de Supabase, hardcodeadas. Kong DB-less no interpola env vars en `key-auth`. | Para LAN aceptable; si se expone `:54321` a internet = takeover de la DB (secret público). | ✅ Resuelto (2026-07-13) — spec `hardening-kong-demo-keys` (done): `supabase/generate-keys.mjs` genera credenciales propias y renderiza `kong.yml` (gitignoreado) desde `kong.yml.template` (versionado) con `envsubst`. `postinstall` mantiene el flujo demo sin fricción. Rotación completa contra un stack reiniciado queda pendiente de probar en un entorno sin dev server activo. |
...
| R13 | `audit_logs` no existía en el esquema de Supabase Cloud (solo local). | No bloqueaba el sync de noches, pero impedía tener auditoría en cloud. | ✅ Resuelto (2026-07-13) — migración corrida a mano en el SQL Editor de Supabase Cloud, verificada con `GET /rest/v1/audit_logs` (200, antes PGRST205). |
...
| R16 | Los tests de integración (`apps/api/tests/integration/`) pegan contra el MISMO Supabase local que usa el dev server — no hay una DB de test aislada. `cleanNightEvents()`/`cleanOrders`/etc. en `db-helpers.ts` borran/crean sin acotar a un rango/prefijo de test, a diferencia de `cleanDrinks()`/`cleanUsers()` (arregladas 2026-07-13 con `TEST_DRINK_ID_FLOOR`/`TEST_USERNAME_PREFIX`, ver spec `restaurar-backup-desde-cloud` y el incidente real de esa sesión donde `drinks`/`users` se vaciaron por esto mismo). | Corriendo la suite de integración repetidas veces en una sesión de desarrollo larga, se acumulan decenas de `night_events`/`orders`/`tickets` de test reales en la base compartida (confirmado: ~49 noches de testing tras una sola sesión). No hay riesgo de perder catálogo/usuarios reales (ya protegidos), pero sí de ensuciar el Historial de Noches con datos falsos si no se corre `cleanup-empty-nights.ts` después. | Abierto — aplicar el mismo patrón de rango/prefijo de test a `night_events`/`orders`/`tickets`, o directamente provisionar una segunda instancia local de Supabase dedicada a tests. |
