...
|---|---|---|---|
| 1 | **Cobro verificado** (`specs/mercadopago/cobro-verificado.md`) | Es el único que pierde plata **en cada uso**. Lo demás son riesgos; esto es daño presente. | ✅ **`done` (2026-07-22)** — implementado, sync a Cloud verificado y **gate físico pasado** (rechazos reales sin ticket ni pedido; cobro aprobado ligado a su pago) |
...
- [x] **Gate físico pasado (2026-07-22)**: tarjeta sin fondos contra el Posnet real → la UI mostró
  el rechazo, sin ticket ni pedido (4 rechazos persistidos en `mp_orders`, 0 pedidos fantasma); y un
  cobro aprobado de $101 quedó ligado a su pago con el monto verificado.

📁 Spec: [`specs/mercadopago/cobro-verificado.md`](./specs/mercadopago/cobro-verificado.md)
(`done`). Riesgo asociado: **R27**, resuelto. **Implementado y gate físico pasado el 2026-07-22**
(backend + frontend + tests + migraciones + push a Cloud verificado con un cierre de noche real). El
