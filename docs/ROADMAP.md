...
**En una línea**: el cobro con Posnet físico funciona de punta a punta y **la venta se concreta
solo si Mercado Pago confirmó el cobro** (`cobro-verificado` `done`, gate físico pasado). Lo que
sigue: el **PR 4** de la remediación MP.
...
> 🎯 **Siguiente: el PR 4** (R27 ya está resuelto por "Cobro verificado", que le pasó adelante).
> Es el que cierra la fuga más peligrosa que dejó a la vista el episodio del Posnet: **hoy hay dos
> sellers activos y el sistema cobra con el "correcto" por casualidad** (R21) — con D9 (vincular
> reemplaza + botón Desvincular) eso se vuelve imposible por construcción, y de paso queda el camino
> limpio para migrar a la cuenta del dueño cuando toque.
>
> Antes de codear: **re-verificar los supuestos del PR 4 contra el estado real** — la sección se
> escribió antes de los hallazgos del 21/22-07 y ya cayeron dos: los cobros salen por el nivel 2
> OAuth (no por el fallback env como asumía el plan), y R24 se reformuló (no es cierto que el
> fallback exija un token de la misma *aplicación* que el lector). La tarea correspondiente ya está
> en la spec como **estrategia F1**: degradar distinguiendo las clases de fallo, guarda de
> **cuenta**, y un preflight de solo lectura al boot que mide si el fallback es utilizable y lo
> muestra en el health y en `/admin`.
...
R27) se sacaron de esta tabla — el detalle de cada resolución está en la spec correspondiente
(R18/R19/R20/R27 en [`cobro-verificado.md`](./specs/mercadopago/cobro-verificado.md)) o en el
historial de git.

> Los riesgos R17-R26 salieron de la auditoría, de la implementación de la remediación MP y de la
> verificación contra la API real de Mercado Pago (21 y 22-07). R28 salió del análisis de R27
