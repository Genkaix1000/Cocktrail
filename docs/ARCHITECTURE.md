...
- **Llaves**: JWT secret demo de Supabase + `anon`/`service_role` firmadas con él. En `supabase/.env.example`; la `service_role` va en `apps/api/.env` como `SUPABASE_SERVICE_ROLE_KEY`. El `docker-compose.yml` trae los valores demo embebidos, así corre sin `.env`.
