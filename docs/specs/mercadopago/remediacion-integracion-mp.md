...
- **Criterios cumplidos**: los bloques A, B, E y F completos (A6 quedó cerrado por construcción con D1 en el PR 4); D y H quedaron cerrados vía `gestion-posnets` (PR 6, 2026-07-23) y G casi completo (queda un resto menor: el nombre de caja hardcodeado en `PdvSection.tsx:430`). Queda solo C (→ PR 5).
...
- [ ] El nombre y el código del punto de venta que se muestran en pantalla son los reales, no literales fijos. → **resto pendiente** (2026-07-24): la creación ya usa formulario real y `PagosSection` muestra el nombre real de la sucursal, pero `PdvSection.tsx:430` todavía hardcodea `"Barra VIP"` como nombre de la caja en la fila del Posnet (`:627`) en vez de leer `storeName`.
